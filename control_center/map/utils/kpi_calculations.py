...
# Helper function to handle KPI display based on selected sheet
def display_kpis(selected_sheet: str, df) -> None:
    if selected_sheet == "Emergency":
        kpis = calculate_emergency_kpis(df)
        display_emergency_kpis(kpis)
    elif selected_sheet == "Workforce":
        kpis = calculate_workforce_kpis(df)
        display_workforce_kpis(kpis)
...
def calculate_emergency_kpis(df):
    ...
    satisfaction_rate = (
        df_copy["Satisfaction"].str.lower().apply(lambda x: x == "satisfied").mean()
        * 100
    )
    ...
    # Calculate unique emergency number
    df_copy["Location"] = df_copy[["Latitude", "Longitude"]].apply(
...
def calculate_workforce_kpis(df):
    ...
    operation_percentage = (
        (active_operations / total_operations) * 100 if total_operations > 0 else 0
    )

    # Calculate evaluation rate
    total_evaluations = df_copy["Evaluation"].sum()
    evaluation_rate = (
        total_evaluations / total_operations if total_operations > 0 else 0
    )
...
def display_emergency_kpis(kpis):
    st.write("## Key Performance Indicators (KPIs)")
    col1, col2, col3, col4, col5 = st.columns([1, 2, 1, 1, 1])
    col1.metric("Closure %", f"{kpis['Closure Percentage']:.2f}%")
    ...
    col5.write(f"Expected Alarm: {kpis['Expected Emergency Alarm']}")

def display_workforce_kpis(kpis):
    st.write("## Key Performance Indicators (KPIs)")
    col1, col2, col3, col4, col5 = st.columns([1, 2, 1, 1, 1])
    col1.metric("Operation %", f"{kpis['Operation Percentage']:.2f}%")
    ...
    col5.write(f"Expected Alarm: {kpis['Expected Complains Alarm']}")
